"""
بوت اقتصاد أحمس v2 - حساب استثمارك مع التضخم
=================================================
...
async def ask_country(chat_id: int, state: FSMContext):
    ...
    await bot.send_message(
        chat_id,
        f"<b>{GOLD_DIAMOND} حساب استثمارك مع التضخم {GOLD_DIAMOND}</b>\n"
        f"{PHARAOH_LINE}\n\n"
...
# ─── معالج الرسائل العامة (بدون أمر /start) ───
@router.message(F.text, ~F.text.startswith("/"))
async def catch_all_message(message: Message, state: FSMContext):
    """لو المستخدم بعت أي رسالة من غير ما يكون في حالة FSM، نبدأ من الأول"""
