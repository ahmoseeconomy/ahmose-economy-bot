...
async def get_gold_price_local(currency):
    ...
    if gu is None or ur is None:
        return None
    c = gu * ur
    ag = DEFAULT_GOLD_ANNUAL_GROWTH / 100
    return {
        "current_gram_usd": round(gu, 2),
        "current_gram_local": round(c, 2),
        "forecast_6m": round(c * (1 + ag * 0.5), 2),
        "forecast_1y": round(c * (1 + ag), 2),
        "forecast_3y": round(c * (1 + ag) ** 3, 2),
...
async def get_hard_currency_data(currency):
    ...
    return {
        "current_rate": round(ur, 4),
        "forecast_6m": round(ur * (1 + ac * 0.5), 4),
        "forecast_1y": round(ur * (1 + ac), 4),
        "forecast_3y": round(ur * (1 + ac) ** 3, 4),
                ...
                if r.status == 200:
                    ...
                    if len(d) > 1 and d[1]:
                        for e in d[1]:
                            ...
                            y = e.get("date")
                            if v is not None:
                                result = {"rate": round(v, 1), "year": y, "source": "World Bank"}
...
async def fetch_all_data(country_code, currency):
    ...
    return {
        ...
        "fetched": gold is not None or hc is not None,
    }
